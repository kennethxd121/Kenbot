...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Kenbot - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Kenbot",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Kenbot in case of issues",
                        action="store_true")
...
def update_red():
    try:
        ...
        return
    if code == 0:
        print("\nKenbot has been updated")
    else:
        print("\nKenbot could not update properly. If this is caused by edits "
...
def reset_red(reqs=False, data=False, cogs=False, git_reset=False):
    ...
    if git_reset:
        ...
        if code == 0:
            print("Kenbot has been restored to the last local commit.")
        else:
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Kenbot (discards code changes, keeps data intact)")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Kenbot...")
                continue
            else:
                if not autorestart:
                    break

    print("Kenbot has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Kenbot without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Kenbot Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        print("1. Run Kenbot /w autorestart in case of issues")
        print("2. Run Kenbot")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Kenbot needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Kenbot cannot work without the pip module. Please make sure to "
